...
Equalizer = (function () {
  ...
  let as_graphic_eq = function (filters, sampleRate) {
    let rawFS = config.GraphicEQRawFrequences,
      fs = config.GraphicEQFrequences;
    let coeffs = filters_to_coeffs(filters, sampleRate);
  ...
  let optimize = function (fr, frTarget, filters, iteration, dir) {
    ...
    let [minQ, maxQ] = config.OptimizeQRange;
    let [minGain, maxGain] = config.OptimizeGainRange;
    let [maxDF, maxDQ, maxDG, stepDF, stepDQ, stepDG] = config.OptimizeDeltas[iteration];
    ...
    if (!dir) {
      ...
      for (let i = 0; i < filters.length - 1; ) {
        let f1 = filters[i];
        let f2 = filters[i + 1];
        if (Math.abs(f1.freq - f2.freq) <= freq_unit(f1.freq) && Math.abs(f1.q - f2.q) <= 0.1) {
          f1.gain += f2.gain;
